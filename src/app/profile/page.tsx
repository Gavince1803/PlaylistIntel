...
export default function ProfilePage() {
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-[#f8fafc] via-[#e5e7eb] to-[#1DB954] dark:from-[#191414] dark:via-[#232323] dark:to-[#1DB954] p-3 sm:p-4 lg:p-6 transition-colors">
      <div className="relative bg-white dark:bg-[#232323] rounded-xl lg:rounded-2xl shadow-2xl border border-[#e5e7eb] dark:border-[#282828] p-4 sm:p-6 lg:p-10 w-full max-w-md mx-2 sm:mx-4 transition-colors">
        <button
          onClick={() => router.back()}
          className="absolute left-2 top-2 sm:left-4 sm:top-4 text-gray-500 dark:text-gray-400 hover:text-black dark:hover:text-white bg-gray-100 dark:bg-[#232323] rounded-full p-2 focus:outline-none focus:ring-2 focus:ring-[#1DB954] text-sm lg:text-base"
          aria-label="Go back"
        >
          ← Back
        </button>
        <div className="flex flex-col items-center mb-6 lg:mb-8 mt-4">
          {avatar ? (
            <img
              src={avatar}
              alt="Avatar preview"
          ...
          <h2 className="text-xl lg:text-2xl xl:text-3xl font-bold text-gray-900 dark:text-white mb-1 mt-2 text-center">
            {profile?.display_name || session?.user?.name || "User"}
          </h2>
          <p className="text-gray-500 dark:text-gray-400 mb-2 text-center break-all text-xs lg:text-sm">
        ...
        <div className="border-t border-[#e5e7eb] dark:border-[#282828] pt-4 lg:pt-6 mt-4 lg:mt-6">
          <div className="flex items-center justify-between mb-3 lg:mb-4">
            <h3 className="text-base lg:text-lg font-semibold text-gray-900 dark:text-white">Your Playlists</h3>
            <button
              onClick={() => router.push('/')}
          ...
          {loading ? (
            ...
            <ul className="space-y-2 max-h-48 lg:max-h-60 overflow-y-auto pr-2">
              {playlists.map((pl) => (
                <li key={pl.id} className="flex items-center gap-2 lg:gap-3 p-2 rounded-lg hover:bg-gray-50 dark:hover:bg-[#2a2a2a] transition-colors cursor-pointer">
                  {pl.images?.[0]?.url ? (
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="font-semibold text-gray-900 dark:text-white text-xs lg:text-sm truncate">{pl.name}</div>
